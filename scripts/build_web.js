...
<div id="tab-generate" class="tab">
  ...
  <div class="card">
    ...
    <p style="color:#555;margin-bottom:12px;font-size:13px">The downloaded CMD contains the Veeam collector source embedded in the file. It writes a local PowerShell script, runs the Veeam query, and packages the report, logs, manifest, and collector source into a TAR archive.</p>
    <div class="stat-grid">
  ...
  <div class="card">
    ...
    <ul style="font-size:13px;line-height:1.8;color:#374151;padding-left:18px">
      ...
      <li>The browser parses the same human-readable collector report used by the Unison analysis flow.</li>
    </ul>
  </div>
</div>
<div id="tab-import" class="tab">
  <div class="card">
    ...
    <div id="dropZone" class="drop-zone" onclick="document.getElementById('fileInput').click()">
      ...
      <p>Individual <code>.txt</code> or <code>.log</code> files can be imported for troubleshooting.</p>
      <input type="file" id="fileInput" multiple accept=".tar,.txt,.log" style="display:none" onchange="handleFiles(this.files)">
    </div>
...
const STATE = { archive:null, reportText:'', consoleLog:'', errorLog:'', parsed:null };
...
function handleTarResult(filename,result){if(result.errors.length){setImportMessage('Archive import failed: '+result.errors.join('; '),'alert-danger');return;}const report=decodeBytes(result.files.get('veeam_collector_report.txt'))||'';const consoleLog=decodeBytes(result.files.get('veeam_collector_console.log'))||'';const errorLog=decodeBytes(result.files.get('veeam_collector_errors.log'))||'';const manifest=parseManifest(decodeBytes(result.files.get('manifest.txt'))||'');STATE.archive={filename,manifest,warnings:result.warnings};STATE.reportText=report;STATE.consoleLog=consoleLog;STATE.errorLog=errorLog;STATE.parsed=parseReport(report);refreshUI();showStatus('Imported '+filename);}
function loadLooseTexts(texts){STATE.archive={filename:'loose files',manifest:{},warnings:[]};STATE.reportText=texts['veeam_collector_report.txt']||Object.values(texts).find(t=>/Veeam Collector Report/.test(t))||'';STATE.consoleLog=texts['veeam_collector_console.log']||'';STATE.errorLog=texts['veeam_collector_errors.log']||'';STATE.parsed=parseReport(STATE.reportText);refreshUI();showStatus('Imported loose files.');}
function clearImport(){STATE.archive=null;STATE.reportText='';STATE.consoleLog='';STATE.errorLog='';STATE.parsed=null;refreshUI();showStatus('Import cleared.');}
...
function parseReport(text){const sections=extractSections(text);const metadata={};const host=/^Host\s+:\s*(.+)$/m.exec(text||'');if(host)metadata.host=host[1].trim();const ps=/^PowerShell\s+:\s*(.+)$/m.exec(text||'');if(ps)metadata.powershell=ps[1].trim();const win=/^Window\s+:\s*(.+)$/m.exec(text||'');if(win)metadata.window=win[1].trim();const jobs=/^Jobs\s+:\s*(\d+)\s+\(Failed:\s*(\d+)\s+Warning:\s*(\d+)\s+Success:\s*(\d+)\s+WithError:\s*(\d+)\)/m.exec(text||'');if(jobs){metadata.jobs=jobs[1];metadata.failed=jobs[2];metadata.warning=jobs[3];metadata.success=jobs[4];metadata.with_error=jobs[5];}
  ...
  return{metadata,sections,sessions:parseSessions(text),definedJobs:parseFixed(sections['Defined Jobs'],[['Job',0,38],['Type',39,50],['On',51,54],['Next / schedule',55,73],['Last run',74,90],['Status',91,102],['Last Result',103,114]]),repositories:parseFixed(sections['Defined Repository'],[['Repository',0,30],['Tier',31,47],['Parent',48,68],['Status',69,81],['Total',82,93],['Used',94,105],['Free',106,117],['Used %',118,125]]),capacity,offloads:parseWhitespaceTable(sections['SOBR Offload Stats'])};
}
function refreshUI(){const p=STATE.parsed;const has=!!(p&&STATE.reportText);document.getElementById('btnReport').disabled=!has;setImportMessage(has?'Collection imported: '+(STATE.archive?.filename||'loose files'):'No collection imported yet.',has?'alert-success':'alert-info');const stats=document.getElementById('importStats');stats.innerHTML=stat('Sessions',p?.sessions.length||0)+stat('Defined jobs',p?.definedJobs.length||0)+stat('Repositories',p?.repositories.length||0)+stat('Capacity rows',p?.capacity?.rows?.length||0);document.getElementById('reportStats').innerHTML=stats.innerHTML;renderPreview();}
...
function renderPreview(){const p=STATE.parsed,el=document.getElementById('importTables');if(!p){el.innerHTML='';return;}el.innerHTML=previewTable('Session Summary',p.sessions.slice(0,20))+previewTable('Defined Repositories',p.repositories.slice(0,20));}
...
function rowsFromObjects(rows){if(!rows||!rows.length)return[['No rows']];const cols=Array.from(rows.reduce((set,r)=>{Object.keys(r).forEach(k=>set.add(k));return set;},new Set()));return[cols,...rows.map(r=>cols.map(c=>r[c]??''))];}
...
function generateReport(){const p=STATE.parsed;if(!p)return;const meta=STATE.archive?.manifest||{};const sheets=[{name:'Cover',rows:[['VeeamStorageTools Report'],['Generated',new Date().toISOString()],['Archive',STATE.archive?.filename||''],['Customer',meta.customer||val('customerName')],['VBR Label',meta.vbr_label||val('vbrLabel')],['Host',p.metadata.host||''],['Window',p.metadata.window||''],['Jobs',p.metadata.jobs||''],['Failed',p.metadata.failed||''],['Warning',p.metadata.warning||''],['Success',p.metadata.success||''],['With Error',p.metadata.with_error||'']]},{name:'Sessions',rows:rowsFromObjects(p.sessions)},{name:'Defined Jobs',rows:rowsFromObjects(p.definedJobs)},{name:'Repositories',rows:rowsFromObjects(p.repositories)},{name:'Capacity Extents',rows:rowsFromObjects(p.capacity?.rows||[])},{name:'Capacity Totals',rows:rowsFromObjects(p.capacity?.totals_by_sobr||[])},{name:'SOBR Offloads',rows:rowsFromObjects(p.offloads)},{name:'Manifest',rows:Object.entries(meta)},{name:'Licensing Raw',rows:rawRows(p.sections['VBR Licensing']||'')},{name:'Backup Versions Raw',rows:rawRows(p.sections['Backup Versions']||'')},{name:'Report Raw',rows:rawRows(STATE.reportText)},{name:'Console Log',rows:rawRows(STATE.consoleLog)},{name:'Error Log',rows:rawRows(STATE.errorLog)}];const bytes=buildWorkbook(sheets);const blob=new Blob([bytes],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});const fn='VeeamStorageTools_'+safeLabel(meta.vbr_label||val('vbrLabel'),'VBR')+'_'+new Date().toISOString().slice(0,10)+'.xlsx';const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=fn;a.click();URL.revokeObjectURL(url);const el=document.getElementById('reportStatus');el.className='alert alert-success';el.textContent='Generated '+fn;el.style.display='block';showStatus('Downloaded '+fn);}
