...
function generateCmdContent(){
  ...
  const hashFileCmd="$ErrorActionPreference='Stop';$p=$env:VST_HASH_FILE;if(-not (Test-Path -LiteralPath $p)){throw 'Hash target not found'};[Console]::Write((Get-FileHash -Algorithm SHA256 -LiteralPath $p).Hash.ToLowerInvariant())";
  ...
  lines.push('SET "COLLECTOR_SHA_EXPECTED='+COLLECTOR_SHA256+'"','SET "COLLECTOR_SHA_ACTUAL="','SET "VST_HASH_FILE=%COLLECTOR_PS1%"','FOR /F "delims=" %%H IN (\'powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command '+psCmd(hashFileCmd)+'\') DO SET "COLLECTOR_SHA_ACTUAL=%%H"','SET "VST_HASH_FILE="','IF /I NOT "%COLLECTOR_SHA_ACTUAL%"=="%COLLECTOR_SHA_EXPECTED%" (','  echo [FATAL] Collector SHA256 verification failed. 1>&2','  echo Expected: %COLLECTOR_SHA_EXPECTED% 1>&2','  echo Actual  : %COLLECTOR_SHA_ACTUAL% 1>&2','  EXIT /B 1',')','');
  lines.push('SET "INVENTORY_SHA_EXPECTED='+INVENTORY_SHA256+'"','SET "INVENTORY_SHA_ACTUAL="','SET "VST_HASH_FILE=%INVENTORY_PS1%"','FOR /F "delims=" %%H IN (\'powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command '+psCmd(hashFileCmd)+'\') DO SET "INVENTORY_SHA_ACTUAL=%%H"','SET "VST_HASH_FILE="','IF /I NOT "%INVENTORY_SHA_ACTUAL%"=="%INVENTORY_SHA_EXPECTED%" (','  echo [FATAL] Inventory collector SHA256 verification failed. 1>&2','  echo Expected: %INVENTORY_SHA_EXPECTED% 1>&2','  echo Actual  : %INVENTORY_SHA_ACTUAL% 1>&2','  EXIT /B 1',')','');
