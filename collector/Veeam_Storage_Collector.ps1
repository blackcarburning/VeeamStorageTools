...
# Veeam StorageTools collector maintained by OpenClaw for blackcarburning.
# Collection-focused fork: no email delivery, no retention cleanup.
# If this banner is missing, you are not running the StorageTools collector.
###############################################################################
...
.PARAMETER OutputPath
    Optional file path where the canonical collector output should also be
...
.EXAMPLE
    .\Veeam_Storage_Collector.ps1 -CollectorDebug
...
.EXAMPLE
    .\Veeam_Storage_Collector.ps1 -Hours 48 -OutputPath C:\Temp\veeam_collector_report.txt

    Generates the normal report and writes the same canonical human-readable
    report body to the requested file for archive packaging.
...
.NOTES
    Usage notes:
      ...
      - This StorageTools collection version does not send email and does not
        remove old report files. Use -OutputPath when a wrapper needs a stable
        report file for archive packaging.
...
function Write-EnvironmentDiagnostics {
    ...
    Write-DebugMessage ('  Arguments      : Hours={0}  Json={1}  OnlyFailures={2}  CollectorDebug={3}  DebugLogPath={4}  OutputPath={5}' `
        -f $Hours, $Json.IsPresent, $OnlyFailures.IsPresent, $CollectorDebug.IsPresent, $DebugLogPath, $OutputPath)
...
# ---------------------------------------------------------------------------
# Write-CollectorOutputPath
#   Writes the canonical collector output to the explicit file requested by the
#   wrapper. No email delivery, retention cleanup, or implicit report directory
#   side effects are performed by this collection-focused version.
# ---------------------------------------------------------------------------
function Write-CollectorOutputPath {
    [CmdletBinding()]
    ...
    try {
        ...
        [System.IO.File]::WriteAllText($fullPath, $Body, [System.Text.Encoding]::UTF8)
        Write-ProgressMessage ('Collector output written to: {0}' -f $fullPath)
        return $fullPath
    } catch {
        Write-Warning ('Unable to write collector output to "{0}": {1}' -f $OutputPath, $_.Exception.Message)
        Write-DebugMessage ('[Write-CollectorOutputPath] Failure:' + [Environment]::NewLine + (Format-ErrorRecord -ErrorRecord $_))
        return ''
    }
}
...
trap {
    $fatalMsg = '[FATAL] Veeam_Storage_Collector.ps1 terminated with an unhandled error.'
...
if ($Json) {
    $collectorOutput = ConvertTo-Json -InputObject @($sorted) -Depth 6
    Write-Output $collectorOutput
    Write-Warning ('Summary: jobs={0} failed={1} warning={2} success={3} with_error={4}' `
        -f $totalJobs, $failedCount, $warnCount, $successCount, $withError)
} else {
    $collectorOutput = $reportBody
    Write-Output $collectorOutput
}
